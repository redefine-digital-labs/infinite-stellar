...
> **Status:** Experimental pre-production testnet canary plus a playable local Round 5 rules sandbox. The canonical Soulidity mainnet `SoulState` ABI, shared-state event discovery, typed ranked-enrollment adapter, and proof-bound enrollment/home/move/move-new transaction gateway are pinned in source, and the complete package passes a Sui mainnet publish dry-run. Infinite Stellar is not deployed on mainnet. The web route resolves an existing deterministic Seat first, otherwise validates current canonical Soul ownership from BCS. For a deployed Seat it restores an independently encrypted chain/package/Season/Seat/controller map namespace, derives Planet object IDs from private location commitments, point-reads exact Planet and pending-Voyage BCS, rejects version races, and merges only coordinates that satisfy the pinned Season geometry. A bounded global event replay remains available for diagnostics. Signing stays unreachable unless every package, proof, audit, operations, and multisig pin is present. Production proof keys, reveal/capture proofs, external Artifact custody, public checkpoint indexer, and production services remain unavailable and fail closed.
...
- **Implementation:** Round 5 gameplay state machines are implemented in Move; canonical Soulidity mainnet enrollment is pinned and undeployed; complete development claim/move/move-new relations constrain manifest-committed MiMC, rarity, radius, Perlin, home-band, distance, and action intent; each Season binds exact per-action circuit-config IDs and digests; real development Groth16 proofs create a Founding Planet, dispatch nonce-bound fleets, and atomically initialize a proof-derived natural Planet in Sui Move tests. The SDK binds proof bytes to the exact sender/Seat/nonce/deadline/Manifest intent, builds all four player PTBs, simulates before wallet signing, persists submitted enrollment digests for exact finality recovery, reconciles finalized BCS events/effects, discovers canonical shared Soulidity state, derives/reads the controller Seat bundle, and validates bounded global plus deterministic private-map Planet/Voyage projections. The React ranked route can render a chain-authoritative interactive map from the controller's authenticated local coordinate vault without uploading coordinates or replaying global history; commands remain sealed. The separately labeled local sandbox remains the only playable strategy authority, and ranked writes remain fail-closed.
- **License:** [MIT](LICENSE)
- **Next gate:** Complete the production proof ceremony and key pinning, provision the rebuildable public checkpoint indexer plus sponsor/monitoring services, connect proof-backed ranked commands to the authenticated map, then pass independent circuit/contract/client audits, multi-wallet soak, operational controls, and release clearance before publishing a playable mainnet season. See [mainnet readiness](docs/17-mainnet-readiness.md).

The Dark Forest v0.6 root game is GPL-3.0 licensed; its renderer and procedural packages have individual MIT licenses. The owner explicitly accepts compatible GPL and MIT reuse with attribution and corresponding-source obligations. The client adapts the pinned original planet fragment shader and biome palette; see [third-party notices](apps/web/public/third-party/NOTICE.md). Do not treat separately licensed upstream code as original Infinite Stellar code.
