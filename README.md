...
> **Status:** Experimental pre-production testnet canary plus a playable local Round 5 rules sandbox. The canonical Soulidity mainnet `SoulState` ABI, shared-state event discovery, typed ranked-enrollment adapter, and proof-bound enrollment/home/move/move-new transaction gateway are pinned in source, and the complete package passes a Sui mainnet publish dry-run. Infinite Stellar is not deployed on mainnet. The web route resolves an existing deterministic Seat first, otherwise validates current canonical Soul ownership from BCS. For a deployed Seat it can reconstruct a bounded, checkpoint-event-derived and object-digest-anchored Manifest/Runtime/Planet/Voyage read model. Signing stays unreachable unless every package, proof, audit, operations, and multisig pin is present. Production proof keys, reveal/capture proofs, external Artifact custody, checkpoint indexer, and production services remain unavailable and fail closed.
