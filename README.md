...
> **Status:** Experimental pre-production testnet canary plus a playable local Round 5 rules sandbox. The canonical Soulidity mainnet `SoulState` ABI, shared-state event discovery, typed ranked-enrollment adapter, and proof-bound enrollment/home/move/move-new transaction gateway are pinned in source, and the complete package passes a Sui mainnet publish dry-run. Infinite Stellar is not deployed on mainnet. The web route now resolves an existing deterministic Seat first, otherwise validates current canonical Soul ownership from BCS, and keeps signing unreachable unless every package, proof, audit, operations, and multisig pin is present. Production proof keys, reveal/capture proofs, external Artifact custody, and production services remain unavailable and fail closed.
...
- **Implementation:** Round 5 gameplay state machines are implemented in Move; canonical Soulidity mainnet enrollment is pinned and undeployed; complete development claim/move/move-new relations constrain manifest-committed MiMC, rarity, radius, Perlin, home-band, distance, and action intent; each Season binds exact per-action circuit-config IDs and digests; real development Groth16 proofs create a Founding Planet, dispatch nonce-bound fleets, and atomically initialize a proof-derived natural Planet in Sui Move tests. The SDK binds proof bytes to the exact sender/Seat/nonce/deadline/Manifest intent, builds all four player PTBs, simulates before wallet signing, reconciles finalized BCS events/effects, discovers canonical shared Soulidity state, and derives/reads the controller Seat bundle. The React mainnet-readiness route uses that chain model; the playable strategy authority remains the labeled local sandbox, and ranked writes remain fail-closed.
- **License:** [MIT](LICENSE)
