...
This PRD defines the first complete player product. Detailed game constants belong in a versioned season rules specification; protocol and security details belong in the linked architecture documents.

## Product statement

Infinite Stellar is a seasonal, hidden-information strategy game in which a current Soul holder creates a temporary commander projection, builds a civilization in a shared universe, proves hidden-geometry actions on Sui, and leaves the finished season with verifiable history but no permanent ranked power.
...
```mermaid
flowchart LR
    W["Current holder"] -->|authorizes| S["Soul + SoulState"]
    A["Animacraft projection"] -->|visual snapshot| P["CommanderProjection"]
    S -->|owner + epoch snapshot| P
    W -->|controls| SS["SeasonSeat"]
    P -->|represents| SS
...
The core multiplayer universe and guest tutorial use dedicated Infinite Stellar product surfaces. Infinite Flow Engine may later host a separate Soul-bound prologue or PvE Scene. Current Infinite Flow requires a canonical Soul/profile and writes persistent independent Run history, so it cannot serve the guest, resettable, history-free tutorial and never becomes the multiplayer Season, Planet, or Arrival authority.
...
3. Transactions are sponsored within declared abuse limits.
...
2. The client lists Souls whose canonical `SoulState` is currently owned by that address.
3. The user selects one eligible Soul.
4. The client explains that the Soul supplies identity, not ranked power.
5. The user selects a public Animacraft projection or a neutral fallback.
6. The user selects one manifest-defined seasonal doctrine/build from the same budget available to every ranked Seat.
7. The client displays the exact visual snapshot, public exposure, doctrine, season rules, transfer behavior, and recovery warning.
8. The user creates one Season Seat and one Commander Projection.

### C. First action

1. The client searches locally for a valid home region.
2. It creates a witness and proof without sending coordinate preimages to required services.
3. The user previews the claim and signs or approves a sponsored transaction.
4. The client shows pending, finalized, indexed, and failed states separately.
5. The user's private map records the new home.
...
| --- | --- | --- | --- |
| ID-001 | P0 | Enrollment validates the canonical Soul package/interface, `SoulState` ID, Soul ID, current owner, current `ownership_epoch`, and `!is_listed` | A forged ID, copied owner, stale epoch, listed Soul, or unsupported package aborts before either ranked slot is consumed |
...
| ID-006 | P1 | Open Agent League uses a separate narrow, expiring, rate-limited command capability | Capability cannot access Soul custody or act outside its mask/term |
...
| --- | --- | --- | --- | --- |
| Current owner enrolls an unlisted eligible Soul | Binding becomes active and snapshots owner/epoch | Fresh fixed-controller Seat starts | Soul and Seat slots are consumed for the season | Show transfer rules before signature |
| Soul is already listed before enrollment | No binding is created | No Seat or ranked slot is consumed | Owner may retry after cancelling the listing | Fail before any partial state and link to listing status |
| Bound Soul is listed after enrollment | Listing alone does not rotate owner/epoch, so attribution remains live | Existing controller and civilization continue | Prospective buyer receives a prominent active-season warning | Official UI warns seller and buyer; listing never implies empire custody |
...
| ID | Priority | Requirement | Acceptance evidence |
| --- | --- | --- | --- |
| VIS-001 | P0 | Enrollment freezes an approved visual reference, content version, commitment/hash, display-license reference, and fallback | Historical projection and its applicable permission resolve deterministically or use fallback |
...
| UNI-004 | P0 | The user can export, encrypt, restore, and verify the local map vault | Recovery drill restores a playable map on a clean browser profile |
| UNI-005 | P0 | Discovering a coordinate does not itself grant ownership | Only a valid home claim or settled colonization arrival changes control |
...
| ACT-001 | P0 | The user can claim a home, create/target a planet, dispatch energy, reinforce, attack, and settle arrivals | End-to-end tests cover every state and exact boundary |
...
| ID | Priority | Requirement | Acceptance evidence |
| --- | --- | --- | --- |
| ELIM-001 | P0 | `CivilizationState` deterministically distinguishes Active, AtRisk, RecoveryEligible, Eliminated, Settled, and Cancelled | Ownership/arrival vectors reproduce every transition without an indexer scan |
| ELIM-002 | P0 | A Seat becomes RecoveryEligible only when it controls no planet, has no qualifying capture arrival, has an unused recovery, and is before effective `recovery_close` | Exact-boundary and stale-state tests reject every ineligible recovery |
...
| ID | Priority | Requirement | Acceptance evidence |
| --- | --- | --- | --- |
| UX-001 | P0 | An eligible Soul holder reaches a finalized home claim within five minutes p50 after ranked Seat creation; tutorial time is measured separately | Instrumented event timestamps and usability sessions meet the cohort definition below |
| UX-002 | P0 | Sponsored transactions hide gas acquisition during tutorial and bounded initial play | New-wallet test completes without a prior SUI balance |
...
| OPS-005 | P0 | A phase boundary whose current effective time has arrived or whose one-way transition has committed is immutable and never reopens; an extension must execute strictly before the earliest boundary it affects | Exact-boundary tests after universe opening, recovery close, Beacon activation, and movement close reject changes to completed phases while permitting only a separately authorized later suffix |
...
| Planets, energy, arrivals, combat, and ScoreCards | Rendering, animation, audio, and cached projections |
...
2. Wallet connection and eligibility.
3. Soul selector with owner/epoch and transfer disclosure.
4. Projection selector with provenance, license, and fallback preview.
5. Equal-budget seasonal doctrine selector.
6. Season rules and consent.
7. Tutorial.
8. Private stellar map.
9. Planet and arrival inspector.
10. Action composer and proof progress.
11. Activity, transaction, and recovery log.
12. Last Light and season timeline.
13. Public leaderboard and evidence view.
14. Settlement, Echo, and optional Chronicle review.
15. Encrypted map-vault export and restore.
...
| First-session tutorial plus ranked home claim | Under 20 minutes p50 |
...
Metrics are segmented by new/returning user, wallet type, device class, proof latency, and season density. No metric collection may violate the privacy boundary to improve observability.

### Metric event and cohort definitions

- **Eligible controller:** a unique controller address that passes ranked enrollment policy and creates at least one valid Season Seat. Internal load-test, operator, sanctioned, and explicitly unranked Seats are excluded by a published allow/deny policy; multiple Seats for one controller count once.
...
- **First finalized home claim:** the first checkpoint-finalized successful ranked home claim for that controller after `SeasonSeatCreated`. The five-minute timer excludes wallet installation, optional tutorial time, and time before the Seat exists, but includes local search, proof, signing, sponsorship, execution, and finality.
...
- **First-session combined time:** measured only in an explicitly consented usability cohort from first tutorial interaction to checkpoint-finalized ranked home claim. The collector receives a duration bucket and completion flag, not a persistent tutorial-to-wallet, Soul, or Seat mapping.
...
Ten internal players complete a 60-minute universe with Soul selection, projection snapshot, home claim, movement, combat, settlement, receipts, and map recovery.
...
The product may accept a versioned public projection and provenance commitments after validating an exact supported release. Season 0 always has a neutral fallback and does not depend on protected-decryption availability or unreleased authoring capabilities.
