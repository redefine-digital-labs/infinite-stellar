...
| Ranked universe projection | SDK scans bounded checkpoint-bearing Planet/Voyage module history, reads exact shared Manifest/Runtime/Planet/Voyage BCS, validates event/object/Season bindings, and fingerprints every object version/digest | Ready as a direct-read fallback and wired to an existing-Seat status route; large histories require the production indexer |
...
| Chain-backed web client | Canonical Soul, deterministic Seat, and bounded Manifest/Runtime/Planet/Voyage reads are chain-backed; the rendered strategy game remains a local deterministic sandbox | Merge exact chain state with the controller's private coordinate vault and production checkpoint indexer while preserving local proof generation |
