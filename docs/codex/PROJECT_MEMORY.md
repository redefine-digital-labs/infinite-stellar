...
- Proof interface v1 has exactly four BN254 public signals and binds a sixteen-field Sui Poseidon action tuple. For `move`, `amount_u64` means maximum route distance; transferred energy and silver remain live-state arguments. Checked-in Circom 2.2.3 `claim_home` and `move` relations now cover manifest-committed radius/rarity/MiMC/Perlin/home/distance geometry. Deterministic TypeScript Arkworks compression is locked against an independent Rust vector and Sui's native verifier. All current keys/artifacts remain development-only; production stays fail-closed until reproducible builds, expanded review, setup ceremony, independent audit, and immutable production-key pinning pass.
