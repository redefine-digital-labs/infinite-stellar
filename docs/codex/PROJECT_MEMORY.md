...
- The typed game SDK and English React client implement the activation journey with deterministic local demo fixtures, checkpoint-shaped finality UX, controller-scoped persistence, and existing-Seat-first resume. Demo screens cannot produce a Sui or Soulidity write. The SDK now constructs canonical enrollment plus proof-bound home/move/move-new transactions, simulates them with checks enabled, rejects resolved execution failures, waits for indexed finality, reconciles exact BCS events/effects, derives the controller Seat from the exact Move typed key, and BCS-validates its Projection/Civilization/Score bundle. Production-verifier readiness remains false and the React ranked route is not yet wired to this gateway, so live writes still fail closed.
